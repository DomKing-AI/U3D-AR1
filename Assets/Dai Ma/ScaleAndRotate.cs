...
public class ScaleAndRotate : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //允许模型最小缩放到0.3 倍最大放大2倍
        if (scale.x > 0.0013f && scale.y > 0.0013f && scale.z > 0.0013f)
        {
            //实用差值运算，模型平滑缩放
            transform.localScale = Vector3.Lerp(transform.localScale, new Vector3(Mathf.Clamp(localScale.x + scaleFactor, 0.0013f, 0.002f),
        Mathf.Clamp(localScale.y + scaleFactor, 0.0013f,
        0.002f),
        Mathf.Clamp(localScale.z + scaleFactor, 0.0013f, 0.002f)), 0.001f);
        }
